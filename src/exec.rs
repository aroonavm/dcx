...
pub fn run_exec(
    ...
    // Pass --container-id for reliable container lookup and --workspace-folder pointing
    // to the original workspace path. The container was already created by dcx up with
    // the correct workspaceFolder, so we just need to ensure exec runs in that context.
