...
out=$("$DCX" clean 2>/dev/null)
code=$?
assert_exit "clean exits 0 removing empty dir" 0 "$code"
assert_contains "clean output mentions removed" "$out" "removed"
assert_dir_missing "empty dir removed" "${RELAY}/dcx-empty-test-00000000"

# --- Safe mode skips active mounts ---
echo "--- safe mode skips active ---"
WS=$(make_workspace)
...
assert_contains "clean output shows orphaned" "$out" "orphaned"
...
out=$("$DCX" clean 2>&1) || true
# Clean should handle the locked dir and still remove the ok one.
